...
/**
  analyze input streetname string and return a list of data.
**/
function parse( streetName ){
  const postal = get_libpostal();

  // use libpostal to expand the address
  var parts = postal.parser.parse_address( streetName );

  /*[ { value: 'via delle vigne', component: 'road' },
     { value: '12', component: 'house_number' } ]*/

  var ret = {};
  parts.forEach(p=>{
    if(p.component=='road') {
      ret.street=street(p.value);
    }else if(p.component=='house_number') {
      ret.number= parseInt(p.value);
    }
    else if(p.component=='city') {
      ret.city=p.value;
    }
  })
...
function housenumber( num ){
  ...
  number = number.replace(/\s+(bis|vic)\s+/gi, '');
  ...
  //separate numbers with slash and return first one 142/143 -> 142
  if( number.match(/^\d+/gm) ) { number=number.match(/^\d+/gm)[0]; }
...
module.exports.parse_address = parse;
